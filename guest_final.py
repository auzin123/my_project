...
class App:
    ...
    def __init__(self, shuffle_questions=False, shuffle_answers=False) -> None:
        ...
        self.time_total = 0
        self.start_time = 0
        self.img_dir = pathlib.Path(__file__).parent / 'image'
    ...
    def show_question(self) -> None:
        ...
        if image_file:
            img = Image.open(self.img_dir / question['картинка'])
            aspect_ratio = img.height / img.width
            new_width = int(500 * aspect_ratio)
            img = img.resize((500, new_width))
    ...
    def on_button(self, button_text) -> None:
        self.clear()
        question = questions[self.quiestion_index]
        if button_text == question['правильный ответ']:
            self.right_anwers += 1
        else:
            self.wrong_anwers += 1
